...
export var StateChart = function (_a) {
    ...
    var machines = props.machine instanceof Array ? props.machine : [props.machine];
    var _machine = toMachine(machines[0]);
    var _d = useState(_machine), machine = _d[0], setMachine = _d[1];
    useEffect(function () {
        setMachine(_machine);
    }, [_machine]);
    ...
    function renderView(current, service) {
        ...
        switch (view) {
            ...
            case 'models':
                return (React.createElement(ListPanel, { service: service, machines: machines, onUpdate: function (machine) {
                        setEvents([]);
                        setResetCount(0);
                        setMachine(machine);
                        var code = (function () {
                            var _machine = toMachine(machine);
                            return typeof _machine === 'string'
                                ? _machine
                                : "Machine(" + JSON.stringify(_machine.config, null, 2) + ")";
                        })();
                        setState(__assign(__assign({}, allState), { machine: machine, code: code, events: [] }));
                    } }));
            default:
                return null;
        }
    ...
    return (React.createElement(StyledStateChart, { className: className, key: code, style: {
        ...
        React.createElement(StyledSidebar, null,
            React.createElement(StyledViewTabs, null, views.map(function (view) {
